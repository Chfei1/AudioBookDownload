    ...
    public partial class DownloadService : IDownloadService
    {
        ...
        public DownloadService(DownloadConfiguration options = null)
        {
            ...
            if (options != null)
            {
                ...
                Package.SaveDirectory = options.SaveDirectory;
            }
            else
            {
        ...
        {
            Package.Status = DownloadStatus.Downing;
            IsBusy = true;
        ...
        public async Task DownloadFileAsync(string address, string fileName)
        {
            try
            {
                ...
                Package.FullName = Path.Combine(Package.SaveDirectory, Package.FileName);
        ...
        protected HttpWebRequest GetRequest(string method, Uri address)
        {
            ...
            request.Headers = Package.Options.RequestConfiguration.Headers;
            request.CookieContainer = Package.Options.RequestConfiguration.CookieContainer;
            request.Host = Package.Options.RequestConfiguration.Host;
            request.ContentType = Package.Options.RequestConfiguration.ContentType;
            request.Timeout = Package.Options.RequestConfiguration.Timeout;
        ...
        {
            var directory = Path.GetDirectoryName(Package.FullName);
            ...
            using var destinationStream = new FileStream(Package.FullName, FileMode.Append, FileAccess.Write);
        ...
        protected void CheckSizes()
        {
            ...
            CheckDiskSize(Package.FullName, Package.TotalFileSize);
            if (Package.Options.OnTheFlyDownload == false) // store temp files on disk, so check disk size
            {
                var doubleFileSpaceNeeded = Directory.GetDirectoryRoot(Package.FullName) ==
